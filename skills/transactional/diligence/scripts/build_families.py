#!/usr/bin/env python3
# ruff: noqa: B904, UP031 -- preserve historical CLI behavior and date formatting.
...
Usage:
    python3 build_families.py --manifest manifest.json --metadata metadata/ \
        ...
        [--model-edges model-edges.json] [--room-root room/]
"""
...
def extract_text(path):
    if path.lower().endswith(".pdf"):
        try:
            ...
            raise OSError("pdftotext not found on PATH")
        if proc.returncode != 0:
            err = proc.stderr.decode("utf-8", errors="replace").strip()
            raise OSError(f"pdftotext failed on {path}: {err}")
        ...
        if src not in paths or dst not in paths:
            ...
            if doc_path not in text_cache:
                try:
                    ...
                    print(f"error: {exc}", file=sys.stderr)
                    sys.exit(1)
            if normalize(quote) not in text_cache[doc_path]:
