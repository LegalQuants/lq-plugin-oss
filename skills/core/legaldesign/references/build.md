...
Only a user-named template authorizes exact import. The shipping v4 templates are `stacked-explainer`, `slide-brief`, `diligence-report`, `method-map`, and `card-hub`; imports read `assets/templates/<name>.template.html`, not filled reference assets. Use `scaffold.py init --form <matching-form> --template <name> --output <workspace>/artifact.html --spec-output <workspace>/artifact.spec.json`, choosing `one-page` or `slide-brief` to match the template rather than forcing one page for every import. New explicit imports use v4; record `strategy: template-import` and `templateRef` and fill safely. Separately supplied older files remain subject to the v2/v3 compatibility rules above.

The `card-hub` presentation is an opt-in one-page recipe: centered title, one framing card containing the overview's context/question/answer units, four peer cards, a full-width concluding/action card, and optional scope footer. Set the sole section's `presentation: card-hub`; retain explicit 12-column placements with the intro units on row 2. Use four span-3 cards on row 3, the action card span 12 on row 4, and scope on row 5. The shared runtime wraps the intro and reflows peer cards to two columns on compact desktop windows. Phones use the shared full-size reader over the preserved overview canvas. Keep each card concise; use purposeful popups for depth without hiding a controlling qualification. Each card has one accessible popup target, not nested links.
