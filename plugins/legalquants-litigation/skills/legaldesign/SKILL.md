...
3. Read [references/component-grammar.md](references/component-grammar.md), the sole UI contract. Inspect relevant packaged examples and supported components, then refine the plan without copying example matter. For a compact overview of peer principles with supporting popups, consider the `card-hub` one-page template; it is not a substitute for a relationship diagram when that relationship is the point.
