...
.current-position { background: var(--badge); color: var(--muted); border: 1px solid var(--line); }
.verification-ok { background: var(--settled); color: var(--settled-ink); border: 1px solid var(--settled-ink); }
.verification-pending { background: var(--proposed); color: var(--proposed-ink); border: 1px solid var(--proposed-line); }
.verification-failed { background: var(--high); color: var(--high-ink); border: 1px solid var(--high-line); }
...
.result-search { display: grid; grid-template-columns: minmax(0, 1fr) auto; gap: 8px 12px; align-items: end;
  background: var(--card); border: 1px solid var(--line); border-radius: 6px; padding: 10px 12px; margin: 0 0 16px; }
.result-search label { font-size: 13px; font-weight: 500; }
.result-search input { display: block; width: 100%; margin-top: 4px; padding: 8px 10px; color: var(--ink);
  background: var(--bg); border: 1px solid var(--line); border-radius: 4px; font: inherit; }
.result-search button { padding: 8px 12px; color: var(--ink); background: var(--badge); border: 1px solid var(--line);
  border-radius: 4px; font: inherit; cursor: pointer; }
.result-search-status { grid-column: 1 / -1; margin: 0; }
...
@media (max-width: 680px) { body { padding-left: 10px; padding-right: 10px; } h1 { font-size: 23px; }
  .result-search { grid-template-columns: minmax(0, 1fr); } .result-search button { justify-self: start; } }
"""
)

SEARCH_JS = r"""
const resultSearch = document.getElementById('result-search');
const clearResultSearch = document.getElementById('clear-result-search');
const resultSearchStatus = document.getElementById('result-search-status');
const issueCards = Array.from(document.querySelectorAll('.issue'));
const decisionCards = Array.from(document.querySelectorAll('#unresolved-strip .finding'));
function applyResultSearch() {
  ...
  decisionCards.forEach((card) => {
    const visible = !query || card.textContent.toLowerCase().includes(query);
    card.hidden = !visible;
    if (visible) visibleDecisions += 1;
  });
  resultSearchStatus.textContent = `Showing ${visibleIssues} review question${visibleIssues === 1 ? '' : 's'} and ${visibleDecisions} decision item${visibleDecisions === 1 ? '' : 's'}.`;
}
resultSearch.addEventListener('input', applyResultSearch);
clearResultSearch.addEventListener('click', () => { resultSearch.value = ''; applyResultSearch(); resultSearch.focus(); });
applyResultSearch();
"""
...
def render_finding(f, docs, source_prefix):
    ...
    if f.get("current_position"):
        out.append('<span class="badge current-position">current position</span>')
    if status == "present":
        quote_status = (f.get("quote_verification") or {}).get("status")
        quote_labels = {
            "confirmed": ("Quote matched to source", "verification-ok"),
            "human-required": ("Quote needs human checking", "verification-pending"),
            "rejected": ("Quote could not be verified", "verification-failed"),
        }
        quote_label, quote_class = quote_labels.get(
            quote_status, ("Quote not yet checked", "verification-pending")
        )
        out.append(f'<span class="badge {quote_class}">{quote_label}</span>')
        checker_status = (f.get("verification") or {}).get("status")
        checker_labels = {
            "confirmed": ("Finding independently checked", "verification-ok"),
            "refuted": ("Independent check disagreed", "verification-failed"),
            "unresolved": ("Independent check needs review", "verification-pending"),
        }
        checker_label, checker_class = checker_labels.get(
            checker_status, ("Independent check pending", "verification-pending")
        )
        out.append(f'<span class="badge {checker_class}">{checker_label}</span>')
    out.append("</div>")
...
def build_html(framework, findings, manifest, source_prefix, review_validation):
    ...
    parts.append(
        ...
        "“Needs a decision” means the available text was incomplete or could not support a reliable answer. "
        "Each Found result also states whether its quote and factual characterization have been checked.</div>"
    )
    parts.append(
        '<div class="result-search" role="search"><label for="result-search">Search questions, agreements, sections, or quoted text'
        '<input id="result-search" type="search" autocomplete="off" placeholder="Try: assignment, renewal, Section 7"></label>'
        '<button id="clear-result-search" type="button">Clear search</button>'
        '<p id="result-search-status" class="small result-search-status" aria-live="polite"></p></div>'
    )
    ...
    for lens in framework.get("lenses", []):
        ...
        for item in lens.get("items", []):
            ...
            parts.append('<article class="issue" data-result-card="true">')
    ...
    parts.append(
        f'<p class="small">Internal control: Gate 2 · framework version {esc(version)}.</p><ul>'
    )
    parts.append(
    ...
    parts.append(f"<script>{THEME_JS}\n{SEARCH_JS}</script></body></html>")
    return "\n".join(parts) + "\n"
