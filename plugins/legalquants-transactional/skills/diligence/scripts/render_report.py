...
"""Render the Gate 3 findings report as one self-contained HTML file.
...
def render_family_summary(families, docs):
    ...
    n_proposed = sum(
        1 for f in fams for e in f.get("edges", []) if e.get("provenance") == "model"
    )
    ...
    if n_proposed:
        out.append(
            '<div class="note">Proposed edges should be zero after Gate 1 confirmation. '
            f"{plural(n_proposed, 'model-proposed edge')} remain unconfirmed; "
...
def build_refusal(result, root_label):
    parts = []
    parts.append("<!doctype html>")
    parts.append(HEAD)
    parts.append("<title>Gate 3: report withheld</title>")
    parts.append(f"<style>{CSS}</style></head><body>")
    parts.append('<header class="strip">')
    parts.append(f"<h1>Gate 3: report withheld for {esc(root_label)}</h1>")
...
def build_html(framework, findings, manifest, families, gaps, result):
    ...
    parts.append("<!doctype html>")
    parts.append(HEAD)
    parts.append("<title>Gate 3: findings report</title>")
    parts.append(f"<style>{CSS}</style></head><body>")

    parts.append('<header class="strip">')
    parts.append(
        f"<h1>Gate 3: findings report for {esc(manifest.get('root_label', 'data room'))}</h1>"
    )
...
def main():
    ap = argparse.ArgumentParser(description="Render the Gate 3 HTML findings report.")
    ...
    result = reconcile_counts.reconcile(manifest, findings, framework, families)
    if not result["ok"]:
        doc = build_refusal(result, manifest.get("root_label", "data room"))
    ...
    doc = build_html(framework, findings, manifest, families, gaps, result)
    with open(args.out, "w", encoding="utf-8", newline="\n") as f:
        f.write(doc)
