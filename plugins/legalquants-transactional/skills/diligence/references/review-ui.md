# Diligence lawyer-facing review UI contract
...
`/legaldesign`, but Diligence does not invoke `/legaldesign` at runtime and does not
depend on its host utilities, network resources, or conversation-only APIs.
`/legaldesign` may consume an approved Diligence result later when the lawyer
...
- Use one status vocabulary everywhere: **Found**, **Not found**, **Needs a
  decision**, and **Needs rendering**. Pair every status color with its text;
...
- The warm-paper background, quiet white/charcoal surfaces, deep green primary
  action, and restrained amber/red status colors are the Diligence house
...
- Use one `LQ · Diligence Review` masthead. Avoid nested-card dashboards,
...
`scripts/review_copies.py` owns this layer. Build `review-copies.json` and its
