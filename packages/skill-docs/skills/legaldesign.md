...
### Shared demo refinements

The packaged `card-hub` one-page template turns the demo composition into a reusable starting point: a centered title, framing card, four peer cards with purposeful popups, a full-width concluding card, and scope note. It is opt-in; issue-oriented slide briefs retain the existing analysis/support layout. The shared editor offers Default, Lavender, Mint, and Sand palettes, saved with the artifact rather than written to firm preferences.

Working and template popups expose a pencil beside Close. Popup-local editing supports text changes, object move/resize/delete, whole-section selection, undo/redo, and frame resizing without a large toolbar consuming the popup. Saved presentation edits do not promote source verification or alter source identity.

Reader HTML exports use the separately compiled client runtime. Export physically removes file-picker/writable-file APIs and generated-file download/save/export code, not merely the controls that expose them. Working copies and sanitized templates retain file saving. Browser regressions reopen real downloads and inspect exported bytes. This reduces unnecessary capability but does not promise that any email provider or antivirus scanner will accept an HTML attachment.
